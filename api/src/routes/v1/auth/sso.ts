...
export class AuthSSORoute {
  ...
  public async getSSOSaml(
    ...
    if(session.getCurrentSessionAccount.logged) {
      if(session.getSsoByNameId(session.getCurrentSessionAccount.username)) {
        currentSessionIndex = session.getSsoByNameId(session.getCurrentSessionAccount.username).sessionIndex;
      } else {
        const newSession = session.addSSO({
          serviceProviderId : 'rutsukun-sso', // The Issuer (Session Participant id)
          nameId: session.getCurrentSessionAccount.username, // NameId Of the logged in user in the SP
  ...
  public async getSSOLogout(
    ...
    session.deleteSso(session.getCurrentSessionAccount.username, 'rutsukun-sso');
    await session.saveSession();
